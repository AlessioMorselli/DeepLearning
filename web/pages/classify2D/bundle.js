...
({
    1: [function(require, module, exports) {
        ...
        function classifyRandomData(numSamples, noise) {
            ...
            for (var points = [], i = 0; i < numSamples; i++) {
                ...
                var y = randomNumber();
                //y += y > 0 ? 0.3 : -0.3;
                var noiseX = (randomNumber() / (noise + 1)),
                    noiseY = (randomNumber() / (noise + 1));
                x += noiseX;
                y += noiseY;
        ...
        function classifySimpleData(numSamples, noise) {
            ...
            for (var points = [], i = 0; i < num; i++) {
                ...
                var y = randomNumber();
                //y += y > 0 ? 0.3 : -0.3;
                var noiseX = noise * Math.random(),
                    noiseY = noise * Math.random();
                x += noiseX;
                y += noiseY;
        ...
        exports.shuffle = shuffle, exports.classifyTwoGaussData = classifyTwoGaussData, exports.regressPlane = regressPlane, exports.regressGaussian = regressGaussian, exports.classifySpiralData = classifySpiralData, exports.classifyCircleData = classifyCircleData, exports.classifyXORData = classifyXORData, exports.classifyRandomData = classifyRandomData, exports.classifySimpleData = classifySimpleData
    }, {}],
    ...
    5: [function(require, module, exports) {
        ...
        function makeGUI() {
            ...
            d3.select("canvas[data-regDataset=" + regDatasetKey + "]").classed("selected", !0),
        ...
        function drawNode(cx, cy, nodeId, isInput, container, node) {
            ...
            var nodeHeatMap = new heatmap_1.HeatMap(30, 10, xDomain, xDomain, div, {
                noSvg: !0
            });
            div.datum({
                heatmap: nodeHeatMap,
        ...
        function drawNetwork(network) {
            ...
            svg.select("g.core").remove(), d3.select("#network").selectAll("div.canvas").remove(),
        ...
        function updateHoverCard(type, nodeOrLink, coordinates) {
            ...
            var value = type === HoverType.WEIGHT ? nodeOrLink.weight : nodeOrLink.bias,
                name = type === HoverType.WEIGHT ? "peso" : "bias";
            hovercard.style({
        ...
        function updateDecisionBoundary(network, firstTime) {
            ...
            var xScale = d3.scale.linear().domain([0, 99]).range(xDomain),
                yScale = d3.scale.linear().domain([99, 0]).range(xDomain),
                i = 0,
                j = 0;
            for (i = 0; i < 100; i++) {
                if (firstTime) {
        ...
        var boundary = {},
            selectedNodeId = null,
            xDomain = [-6, 6],
            heatMap = new heatmap_1.HeatMap(300, 100, xDomain, xDomain, d3.select("#heatmap"), {
                showAxes: !0
            }),
